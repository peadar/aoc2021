...
int main() {
   ...
   std::transform(std::istream_iterator<std::string>(std::cin), std::istream_iterator<std::string>(),
                  ...
                  [&maxbits](const std::string &s){maxbits = std::max(maxbits, s.size()); return std::stol(s, 0, 2);});
   ...
   for (int pass = 0; pass < 2; ++pass) {
      ...
      for (Value key = Value(1) << (maxbits-1); bottom[-1] != *top; key /= 2) {
         auto partition = std::partition_point(top, bottom, [key] (auto a) { return (a & key) == 0; } );
